...
#### 4 - GET PREDECTIONS ON ALL IMAGES IN STRING
def getPredectionStr(boxes,model):
    result = []
    cnt = 0
    for image in boxes:
        ## PREPARE IMAGE
        ...
        if probabilityValue > 0.6:
            cnt = 0
            result.append(repr(classIndex[0]))
        else:
            cnt += 1
            if cnt == 15:
                return result
            result.append(repr(0))
    return result
...
#### 4.1 - TO SPLIT THE IMAGE INTO 81 DIFFRENT IMAGES
def splitBoxesSpecial(img):
    rows = np.array_split(img, [1,8])
    boxes=[]
    for r in rows:
        cols= np.array_split(r, 9)
        for box in cols:
            boxes.append(box)
    return boxes

def resetStatus(delay, status, timer):
    timer += 1
    if timer > 70:
        timer = 0
        status = False
        delay = 0
    return delay, status, timer

def delayHandle(delay):
    delay += 1
    return delay
